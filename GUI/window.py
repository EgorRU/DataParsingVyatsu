...
def open_file_Scopus_left():
    ...
    dlg = tkinter.filedialog.Open(
        filetypes=ftypes, title="Выберите файл", initialdir=os.path.abspath(__file__)
    ...
    if len(filename) > 0:  # если не пустое имя файла
        mainmenu.entryconfigure(
            3, state=NORMAL
        )  # разблокирование кнопки сравнить данные
        ...
        table_left = ttk.Treeview(
            frametableleft, show="headings"
        ...
        )  # штука которая увеличивает таблицу в зависимости от кол-ва строк

        for (
            row
        ) in (
            lst
        ):  # для каждой строки указываем что нет родителя (обязательная тема чтоб было красиво)
            table_left.insert(
                "", tkinter.END, values=row
            )  # без неё слева будет большой пропуск т к предуматривается полноценная иерархия
        list_scopus = []
...
def open_file_Scopus_right():
    ...
    if len(filename) > 0:  # если не пустое имя файла
        ...
        if right_table_create == True:
            table_right.destroy()
        heads = ["author", "title", "year", "link", "citation"]  # столбики
        table_right = ttk.Treeview(
            frametableright, show="headings"
        )  # инициализация таблицы
...
def open_file_Ipublishing_left():
    ...
    if len(filename) > 0:  # если не пустое имя файла
        ...
        for i in range(len(list_ipublishing)):
            lst.append(
                (
                    list_ipublishing[i].author,
                    list_ipublishing[i].title,
                    list_ipublishing[i].year,
                    list_ipublishing[i].article,
...
filemenu_load_WoF.add_command(
    label="Загрузить в левую таблицу", command=open_file_WoS_left
)
filemenu_load_WoF.add_command(
    label="Загрузить в правую таблицу", command=open_file_WoS_right
)
...
filemenu_load_Scopus.add_command(
    label="Загрузить в левую таблицу", command=open_file_Scopus_left
)
filemenu_load_Scopus.add_command(
    label="Загрузить в правую таблицу", command=open_file_Scopus_right
)
...
filemenu_load_Elibrary.add_command(
    label="Загрузить в левую таблицу", command=open_file_Elibrary_left
)
filemenu_load_Elibrary.add_command(
    label="Загрузить в правую таблицу", command=open_file_Elibrary_right
)
...
filemenu_load_Ipublishing.add_command(
    label="Загрузить в левую таблицу", command=open_file_Ipublishing_left
)
filemenu_load_Ipublishing.add_command(
    label="Загрузить в правую таблицу", command=open_file_Ipublishing_right
)
...
scroll_pane.pack(side=tkinter.RIGHT, fill=tkinter.Y)
table_left_temp.pack(
    expand=tkinter.YES, fill=tkinter.BOTH
)  # штука которая увеличивает таблицу в зависимости от кол-ва строк


heads = [" ", " ", " ", " ", " "]  # столбики
table_right_temp = ttk.Treeview(
    frametableright, show="headings"
)  # инициализация таблицы
